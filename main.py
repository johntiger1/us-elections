...
def constructEnsemble(models, parties):
	...
	for model in models:
		...
		for stateData in reader:
			...
			for party in parties:
				voteShare = float(stateData["{}{}".format(\
						"VoteShare", party)])
				winProb = float(stateData["{}{}".format(\
						"WinProb", party)])
				voteSharesIsWhole = \
						voteSharesIsWhole and voteShare >= 0
				winProbsIsWhole = \
						winProbsIsWhole and winProb >= 0
